...
import { ArrowLeft, ArrowRight, CheckCircle2, CreditCard, Eye, EyeOff, ListChecks, TrendingUp, Wallet } from 'lucide-react';
...
export const DashboardPage = () => {
  ...
  const isEmpty = !snapshot.isLoading && !debts.isLoading && snap !== undefined && snap.total_income === 0 && openDebts.length === 0;
  ...
  return (
    <PageTransition>
      <div className="flex flex-col gap-6 w-full">
        ...
        {isEmpty ? (
          <div className="card bg-base-200 border border-primary/15 p-8 flex flex-col gap-5">
            <div>
              <p className="font-semibold text-base-content">Nenhum dado cadastrado ainda</p>
              <p className="text-sm text-base-content/50 mt-1">Configure sua conta em 3 passos para ver o painel completo.</p>
            </div>
            <div className="flex flex-col gap-2">
              {[
                { to: '/income', Icon: Wallet, label: 'Cadastre sua renda' },
                { to: '/debts', Icon: CreditCard, label: 'Registre suas dívidas' },
                { to: '/fixed-expenses', Icon: ListChecks, label: 'Adicione seus gastos fixos' },
              ].map(({ to, Icon, label }) => (
                <Link
                  key={to}
                  to={to}
                  className="flex items-center gap-3 px-4 py-3 rounded-lg border border-base-300 hover:border-primary/40 hover:bg-primary/5 transition-colors group"
                >
                  <div className="w-7 h-7 rounded-md bg-primary/10 flex items-center justify-center shrink-0">
                    <Icon size={14} className="text-primary" />
                  </div>
                  <span className="text-sm font-medium text-base-content">{label}</span>
                  <ArrowRight size={13} className="ml-auto text-base-content/25 group-hover:text-primary transition-colors" />
                </Link>
              ))}
            </div>
          </div>
